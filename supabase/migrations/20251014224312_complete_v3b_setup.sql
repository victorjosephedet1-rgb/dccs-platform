/*
  ...
  1. New Tables
    - `profiles` - User profiles (extends auth.users)
      - id, email, name, role (artist/creator), timestamps
    - `audio_snippets` - Music tracks for licensing
      - id, title, artist, artist_id, duration, price, mood, bpm, genre, audio_url, waveform_data, is_licensed, timestamps
    - `snippet_licenses` - License purchases
      - id, snippet_id, user_id, price_paid, license_type, created_at, valid_until
  ...
  4. Performance
    ...
    - Unique constraint on user/snippet combination for licenses
*/
